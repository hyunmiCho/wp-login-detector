...
add_action('added_option', function($option, $value ) {
    ...
    {
        if($_POST['option_page'] === 'criminalip-test-options-group')
        {
            update_criminalip_config("S",array("limit_sec" => $_POST['criminalip_test-settings']['num_of_failed_logins_by_IP_before_mitigation_starts'] ?? null,"limit_min" => $_POST['criminalip_test-settings']['num_of_failed_logins_by_IP_before_mitigation_starts'] ?? null,"limit_times" => $_POST['criminalip_test-settings']['mins_to_block_ip'] ?? null, "block_hour" => $_POST['criminalip_test-settings']['never_lockout_trusted_users'] ?? null,"api_key"=>$_POST['criminalip_test-settings']['api_key'] ?? null));
        }
    }}, 10, 2);
...
function update_criminalip_config($c_type, $arr) {
    global $wpdb;     
    $table = $wpdb->prefix.'criminalip';
    $data = array('c_type' => $c_type, 'c_useyn' => 'Y', 'c_data'=> json_encode($arr)); 
    $format = array('%s');

    // select count
    $count = $wpdb->get_var("SELECT COUNT(*) FROM {$table} WHERE c_type = '{$c_type}' AND c_useyn = 'Y'");
    if($count == 0){
        $wpdb->insert($table, $data, $format);
        return;
    }
    // update
    $wpdb->update($table, $data, [ 'c_type' => $c_type, 'c_useyn' => 'Y' ], $format);
}
...
// csson - 관리자 초기화 시점에 register_option 실행
add_action('admin_init','source_criminalip_init'); 
...
// csson - 관리자 초기화 시점에 option 등록
function source_criminalip_init() {
...
add_filter('plugin_action_links_'.plugin_basename(__FILE__), 'criminalip_plugin_action_links');
function criminalip_plugin_action_links( $links ) {
    array_unshift($links, '<a href="' . admin_url( 'admin.php?page=source_criminalip_index' ) . '">' . __( 'Settings','criminalip_settings') . '</a>');
